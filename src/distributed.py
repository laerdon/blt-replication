...
# cluster env mgmt / slurm handling, process launch setup
import os
import random
import shutil
import signal
...
# for no recompute ops (saves memory by recomputing some activations in backward pass)
import xformers.ops
# xformers = optimized transformer building blocks
from pydantic import BaseModel, ConfigDict
# pytorch distributed apis (reduction like min/max/sum/avg, process groups handling, etc)
from torch import distributed as dist
from torch.distributed import ReduceOp
# fully sharded data parellel (shards model weights/gradients/optimizer state across GPUs for memory saving)
# mixed precision policy = bit formatting (like minifloat from 3410)
...
# device mesh (higher level abstraction that manages GPU process groups)
# like 1 dim for data parallel sharding, another for tensor parallel
from torch.distributed.device_mesh import DeviceMesh, init_device_mesh
# each gpu has 1) full copy of model, gradients are synchronized across processes (by averaging)
from torch.nn.parallel import DistributedDataParallel as DDP
# lets u select what to checkpoint or recompute
...
# for selective AC (checkpoint most stuff, but don't recompute these expensive ops)
default_no_recompute_ops = {
    ...
    # dstributed communication
    torch.ops.c10d_functional.reduce_scatter_tensor.default,
    # flash attn fwd kernel
...
# data container for distributed behavior
class DistributedArgs(BaseModel):
    model_config = ConfigDict(extra="forbid")
    dp_shard: int = (
        1  # In how many shard to split the model weight. Typically number gpu in a node. FDSP shard dim.
    )
    dp_replicate: int = (
        1  # How many times to replicate the model weight. Typically number of nodes. 1 replica / data parallel group
    )
    # tensor parallel size (>1 => indiv layers/tensors split across GPUs)
    tp_size: int = 1
    selective_activation_checkpointing: bool = False
    # do torch.compile or na
    compile: bool = False
    ...
    def configure_world(self):
        """
        checks whether dp_shard * tp_size == world_size (# of processes)
        """
        pass
...
def get_device_mesh(distributed_args: DistributedArgs):
    """
    describes how gpus are arranged
    """
...
# wrappers for dist.all_reduce (takes a local scalar from each GPU and applies {op} across distributed group)
...
# dist mean for every val in dictionary {x}
...
# rank helpers (gets distributed state from env vars)
...
# master = one process thats like rendezvous point (master_addr / port)
@lru_cache()
...
def setup_env(env_args: EnvironmentArgs):
    """
    applies env_args to os.environ
    """
...
# lets file access nested module by string path
...
# debug for empty tensor / nan / infs /sus stuff
@torch.no_grad()
...
# temp removes all env vars and restores after
...
def parallelize_model(
    ...
    """
    takes normal pytorch model and applies the distr memory strategy to it
    """
